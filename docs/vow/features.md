...
- [x] **[GUEST-SEATING-DECLINED-EXCLUSION] Exclude Declined Guests from Unassigned Seating Group (`GuestListManager.tsx`):** When viewing the guest registry grouped "BY SEATING TABLE", guests with `RSVP Status: Declined` without assigned tables are strictly omitted from the "UNASSIGNED SEATING" table cluster, ensuring unassigned counts only reflect guests actually attending or awaiting RSVP.
...
- [x] **[SHARE-2] Guest Photo & Video Upload Portal (`/upload/[token]`):** Mobile portal routing guest photos and videos directly into the couple's personal Google Drive folder (`My Drive/Wedding Planning/Guest Uploads`) with MIME/extension format validation and a clear link expiration date/time indicator in the footer.
...
- [x] **[PHOTO-GUESTBOOK-OFFLINE-AUTH] Permanent Offline Refresh Token Guarantee & Proactive Auto-Refresh (`route.ts`, `client.ts`, `firestoreDb.ts`, `register-token/route.ts`, `vow/page.tsx`):**
...
- [x] **[SHARE-4] Spouse & Partner Co-Planning Access, Multi-Channel Share Picker & Real-Time URL Sync:**
  ...
  - **Multi-Channel Share & Invite Picker (`AdvancedSettingsModal.tsx`, `ShareModal.tsx`)**: Replaced static "Open Personal Email App" with an interactive "📲 SHARE INVITE" action featuring a multi-channel sharing picker. Enables couples to send the invite via **Text / SMS** (`sms:`), **Personal Email** (`mailto:`), **WhatsApp / Web Messaging** (`wa.me`), and the native **Web Share API / Device Share** (`navigator.share`), alongside 1-click clipboard link copying.
  - **Spreadsheet ID URL Synchronization & Preservation (`/vow`, `switchTab`)**: Guaranteed that the workspace invitation link and active browser address bar strictly retain `?spreadsheetId=${activeSheetId}` regardless of tab switches (e.g. `/vow?spreadsheetId=...#tasks` instead of stripping down to `/vow#tasks`), ensuring bookmarks and copied URLs remain valid workspace invitations.
  - **Instant Partner Access & Invite Handshake (`/vow`, `/api/auth/google/callback`)**: When an invited partner opens an invite link (`/vow?spreadsheetId=...`), renders the dedicated `UnauthenticatedLanding` Co-Planner Invitation view instead of prematurely activating onboarded mock state. On 1-click Google Sign-In, the OAuth callback automatically links the co-planner email, grants Drive writer permissions, and securely returns workspace confirmation—connecting the partner directly to the live workspace without ever sending them to the Etsy order activation or Day 1 setup screens.
  - **Partner Recognition & Shortcut in Activation Portal (`/activate`)**: Detects existing workspace collaborator access when a user or partner arrives on the activation page and renders a 1-click "Enter Workspace" jump banner bypassing the Etsy Order ID verification. Also fixed partner email persistence in `handleSetupComplete` to properly record Partner 2 (`admin2Email` / `partner2.email`).
...
- [x] **[FINANCIALS-MASTER-DETAIL-VIEW] Desktop Master-Detail Split-View for Wedding Financials (`BudgetLedgerManager.tsx`):**
  ...
  - **Master Rail (Left Column - `lg:col-span-5`)**: Fixed rail header with title, live category counter, inline `+ NEW CATEGORY` button, and `FILTER CATEGORIES...` search input anchored permanently at the top (`flex-shrink: 0; position: sticky; top: 0; z-index: 10; background: var(--color-bg)`). Bounded scrollable category budget list (`flex: 1 1 auto; min-height: 0; overflow-y: auto`) scrolls smoothly underneath without hiding the header or search tools. Features distinct active card selection styling (`2px solid var(--color-primary)`, subtle background highlight, and ring shadow). Displays serif category headings, mini utilization progress tracks, BUDGET / SPENT / REMAINING tabular numbers, and item counters. Clicking any card updates `selectedCategoryId`.
...
| **[BUG-8]** | Re-authentication Loop on Modal Edit (`sessionCheck.ts`, `client.ts`, `api/auth/session`, `vow/page.tsx`) | Repeated login prompts when editing guests/vendors caused by `getGoogleAuthAsync` using stale expired `tokenDoc.accessToken` from database over the caller's fresh `accessToken`, coupled with `/api/auth/session` missing spreadsheet ID context. Resolved by strictly prioritizing caller `accessToken`, passing `spreadsheetId` in `handleReauth`, calling `invalidateSessionCheckCache()`, and ensuring only genuine 401 auth errors trigger session expiration. | High | Resolved | 2026-09-12 |
| **[BUG-9]** | Photo Shot List Mobile FAB & Tabs Polish (`PhotoShotListManager.tsx`, `MobileFAB.tsx`) | Mobile tabs rendered duplicated text (`PHOTOGRAPHER SHOT LISTSHOT LIST`) due to uncompiled Tailwind visibility utilities. MobileFAB was obscured by the 150 z-index bottom bar and `<style jsx>` Next.js 16 incompatibilities. Resolved with dedicated responsive `.photo-tab-full-title` / `.photo-tab-short-title` styles, 50/50 pill segmented control, standard `<style>` tag, elevated `zIndex: 160`, and Guestbook speed-dial action. | Medium | Resolved | 2026-09-12 |
| **[BUG-10]** | Guest Registry Seating Table Declined Inclusion (`GuestListManager.tsx`) | In "BY SEATING TABLE" grouping view, guests who declined RSVP without table assignments were incorrectly placed into the "UNASSIGNED SEATING" cluster, artificially inflating the "NEED TABLES" count. Resolved by strictly excluding declined guests without tables from the unassigned seating map. | Medium | Resolved | 2026-09-12 |
| **[BUG-11]** | Guest Registry Reset Filters Mobile Bleed (`GuestListManager.tsx`) | When quick filters were activated, the "RESET FILTERS" button shared a single flex row with two filter selects, causing horizontal overflow and button bleeding on mobile viewports. Resolved by moving active filter tags and the Reset Filters button into a dedicated responsive row with full-width mobile touch target. | Medium | Resolved | 2026-09-12 |
| **[BUG-12]** | Overnight Guestbook Photo Upload Google Drive Auth Failure (`client.ts`, `firestoreDb.ts`, `google/route.ts`, `upload/[token]/route.ts`) | Guestbook photo uploads worked initially but failed overnight in PROD with 502 `"Could not connect to Google Drive"`. Caused by 1-hour Google OAuth access token expiration without refresh token persistence, `sheet2suite-prod` project mismatch in `firestoreDb.ts` that caused ephemeral container fallback, and lack of proactive token refresh in `getGoogleAuthAsync`. Resolved by enforcing `prompt=consent`, proactive token refresh with Firestore writeback, accurate `sheet2suite-7036e` project ID resolution, non-destructive disk merge, and Drive 401 retry resilience. | Critical | Resolved | 2026-09-18 |
| **[BUG-13]** | Redundant Double Plus Button Symbols (`BudgetLedgerManager.tsx`, `KanbanBoard.tsx`, `GoogleDrivePickerModal.tsx`, `TimeDialPicker.tsx`) | Action buttons rendered redundant double plus symbols (e.g. `+ + ADD EXPENSE` and `+ + ADD TASK`) where Lucide `<Plus />` or `<FolderPlus />` icons were immediately followed by literal `+` prefixes in the button text strings. Resolved by performing a comprehensive scan across all 98 files, eliminating all redundant `+` text prefixes from action buttons, mobile FAB labels, and modal titles. | Low | Resolved | 2026-09-18 |
| **[BUG-14]** | Frequent Production Re-authentication & Session Dropouts (`refresh/route.ts`, `sessionCheck.ts`, `client.ts`, `session/route.ts`, `sync/route.ts`, `vow/page.tsx`) | Users in production faced frequent "Session Expired" re-authentication popups caused by Google OAuth access tokens expiring strictly after 1 hour (3600s), absence of an HTTP-only refresh token cookie (`s2s_refresh_token`), serverless Cloud Run container scale-downs dropping ephemeral local token state, and lack of silent refresh before surfacing modal alerts. Resolved by setting 90-day `s2s_refresh_token` HTTP-only cookies, creating a dedicated `/api/auth/refresh` endpoint, adding automatic 401 retry resilience across `/api/sync` and `/api/auth/session`, synchronizing fresh access tokens back to `localStorage`, and implementing proactive background refresh on tab visibility and a 25-minute interval. | Critical | Resolved | 2026-09-24 |
