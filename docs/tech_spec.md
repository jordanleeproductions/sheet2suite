...
- **Mobile Ergonomic Thumb-Zone Navigation (`[NAV-MOBILE-THUMB]`, `[NAV-SWIPE]`, `[NAV-HAPTIC]`):**
  ...
  - **Persistent "New Version Available" Update Toast Notification (`[SYS-UPDATE-TOAST]` - Backlog):** A sticky Monarch-style toast notification pinned to the bottom-right corner of the viewport (`position: fixed; bottom: 1.5rem; right: 1.5rem; zIndex: 9999`) informing active user sessions when a newer production deployment is ready. Powered by a client polling hook (`useAppVersionCheck.ts`) querying a lightweight un-cached `/api/version` endpoint every 10–15 minutes and on window tab focus (`document.addEventListener('visibilitychange')`). Compares server build timestamp/ID against client-loaded build info. Renders a persistent glassmorphism notification card with a refresh icon, update description, and 1-click `[ 🔄 Refresh App ]` (`window.location.reload()`) with a snooze/dismiss option. On mobile viewports, automatically docks safely above the 62px mobile bottom navigation bar and FAB.
...
- **Advanced Settings & Configuration Portal (`AdvancedSettingsModal.tsx`):**
  ...
  - 🛡️ **Security & Access Control:** Read-only vendor portal summary, spouse/partner co-planning admin invitation input with quota enforcement (max 2 co-planners per workspace via `/api/share/partner`), interactive multi-channel sharing picker (**Text / SMS**, **Email**, **WhatsApp / Web Messaging**, and native **Web Share API / Device Share**), real-time browser URL synchronization preserving `?spreadsheetId=...` across tab switches, tokenized collaborator link generation with localStorage fallback, and automatic Google OAuth permission handshake granting Drive edit access and instant workspace access without Etsy activation prompts. Master workspace disconnect button included.
  ...
  - 🔒 **Session Expired & Re-Authentication Modal (`[AUTH-SESSION-EXPIRED-MODAL-REDESIGN]`):** Dedicated security modal triggered on Google OAuth token timeout (`s2v:session-expired`). Features a clean top-right 'X' close button replacing the crowded side-by-side Dismiss button, a prominent full-width `SIGN IN WITH GOOGLE` primary action with official Google SVG branding and loading spinner state, an amber security badge with Lucide `Lock` icon, connected user email indicator, backdrop blur click-to-dismiss, and graceful workspace disconnect option.
...
- **Guest Selection Entree Statistic Cards & Relational Filter Redirection (`[MENU-GUEST-ENTREE-STATS-AND-FILTER-REDIRECT]`):**
  - Dedicated interactive row of statistic cards situated under the main KPI summary bar in `MenuSetupManager.tsx` showing the name and live order count for each Guest Selection Entree (`isGuestChoice !== false`).
  - Clicking any entree stat card or card footer button immediately switches to the **Guest List** tab with `mealFilter` set to that specific dish, highlighting the active filter pill and displaying matching guests.
  - Removed technical catalog SKU ID badges (`item.id`) from user-facing menu cards.
  - Conditionally displays the guest order count badge on menu cards strictly when an item is designated for individual guest choice (`isGuestChoice !== false`), showing "Buffet / Shared Course" for non-selectable dishes.
- **Dynamic Catering Vendor Guest List Spreadsheet Engine (`[CATERING-GUESTLIST-SHEET]` - Backlog):** Dynamically generates a dedicated catering spreadsheet / export tailored for the catering vendor with 7 standard columns: `#` (index/count), `First Name`, `Last Name`, `Age Category` (for kids menu vs adult portioning), `Dietary Restrictions` (allergies and dietary flags), `Meal Choice`, and `Table Number`. Enforces table resolution rule: strictly resolves internal table IDs (e.g. `table-1788572224912`) to the user's human-designated table name or number (e.g. `Table 1`, `Table 2`, `Sweetheart Table`), and filters out declined RSVPs. Integrated into Catering Manager and the Tokenized Catering Share Portal (`/share/[token]`).
...
- **Desktop Master-Detail Split-View (`[FINANCIALS-MASTER-DETAIL-VIEW]`):**
  ...
  - **Master Rail (Left Column - ~5 cols / `lg:col-span-5`)**: Fixed rail header with title, live category counter, inline `+ NEW CATEGORY` button, and `FILTER CATEGORIES...` search input anchored permanently at the top (`flex-shrink: 0; position: sticky; top: 0; z-index: 10; background: var(--color-bg)`). Scrollable category budget list bounded to viewport (`flex: 1 1 auto; min-height: 0; overflow-y: auto`) prioritized by active/alert allocations. Features prominent active selection highlighting (`selectedCategoryId`), serif category titles, mini utilization progress tracks, and BUDGET / SPENT / REMAINING tabular metrics. Clicking any card updates selection.
  - **Bi-Directional Category Sorting (`[FINANCIALS-SORT-BIDIRECTIONAL]`):** Master category sort dropdown provides bi-directional sorting across all dimensions: Alphabetical (`A → Z` and `Z → A`), Target Budget (`High to Low` and `Low to High`), Spent Expenses (`High to Low` and `Low to High`), and Remaining Headroom (`High to Low` and `Low to High`), persisted in `localStorage` (`'s2v_budget_master_sort'`) with backward compatibility for legacy sort keys.
  ...
  - **Desktop Active & Alert Category Subcards (`[BUDGET-ACTIVE-CATEGORY-SUBCARDS]`):** Replaced cramped, unreadable desktop pill buttons with an interactive, responsive subcard grid (`grid-template-columns: repeat(auto-fill, minmax(130px, 1fr))`). Each subcard displays: (1) bold category title with text truncation and `AlertTriangle` icon if over-budget; (2) prominent `1.2rem` bold tabular percentage (`stat.percent%`) with adjacent spent/over amount; (3) dynamic micro progress bar; and (4) elevated dark/contrast selected state (`var(--color-primary)`) that dynamically coordinates with the detail ledger split-view. Clicking an active card switches or toggles selection with smooth hover transitions.
...
- **Desktop Edit Modal Event Hardening (`[BUDGET-MODAL-FREEZE-FIX]`):** Removed synchronous `setState` and unsupported `.select()` calls from `onFocus` handlers on `type="number"` inputs to eliminate focus-thrashing render loops in Chromium browsers. Elevated modal overlay `zIndex` to `1100`, added comprehensive `Escape` key listeners covering all budget/expense edit and delete dialogs, added backdrop click dismissal with `e.stopPropagation()` on content, and synchronized body scroll locking (`document.body.style.overflow = 'hidden'`).
- **Desktop Expense Table Bottom Quick-Add Action Bar (`[FINANCIALS-DESKTOP-TABLE-BOTTOM-ADD]`):** Bottom action bar anchored beneath the Desktop detail expense table displaying recorded expense count and a prominent `+ ADD EXPENSE` button, enabling quick entry for users scrolled to the bottom of large expense tables without having to scroll back to the top of the detail ledger card.
...
- **Multi-Assignee & Multi-Role Selection Engine (`[SCHED-MULTI-ASSIGNEE]`):** Add/Edit Timeline Moment modal supports assigning multiple roles and people to a single itinerary milestone via an interactive multi-select chip interface, custom free-text combobox, and 1-click popular suggestion badges (+ Photographer, + Videographer, + Bridal Party, + Planner, etc.). Stores roles as clean comma-separated values in Google Sheets Column E (`Responsibility / Vendors`), maintaining 100% backward compatibility. Event cards and the UP NEXT active banner render individual styled role badges with 1-click filter shortcuts.
- **Desktop & Mobile Modal Redesign (`[TIMELINE-MODAL-REDESIGN]`):** Completely overhauled the Add/Edit Timeline Moment modal popup. Desktop width expanded from a cramped 520px to a spacious 820px (`max-width: 820px`), eliminating truncated text inputs and cramped columns. Organized into 4 distinct elevated cards: (1) **Timing & Duration** with a dynamic live duration calculation badge (`⏱ EST. DURATION: X hrs Y mins`), midnight rollover detection, and dedicated sub-rows for `:00, :15, :30, :45` presets and `+30m, +45m, +1h, +1.5h, +2h` duration chips; (2) **Moment & Venue Location** side-by-side inputs; (3) **Assigned Roles & Vendors** with dedicated search bar, `+ ADD` custom pill button, quick-add popular role pills, and expandable role directory; (4) **Notes & Vendor Guidelines** card with icon and clear layout. Redesigned sticky footer with high-contrast `DELETE` button (`#dc2626` background, `#ffffff` text, `Trash2` icon), `CANCEL`, `SAVE & ADD NEXT`, and primary `SAVE MOMENT / SAVE CHANGES`. Mobile view automatically converts into a fluid 1-column stack with touch-friendly tap targets and safe-area padding.
- **Time Input Compact Width & High-Contrast Duration Pill (`[TIMELINE-MODAL-POLISH-SPACING-CONTRAST]`):** Shortened time textboxes in `TimeDialPicker.tsx` (`max-width: 185px; flex: 0 1 185px`) to eliminate excessive empty whitespace and position the time input, DIAL button, and AM/PM selector in a cohesive visual group. Enhanced the estimated duration pill with rich, high-contrast dark amber typography (`#92400e`, WCAG AAA 8.6:1 contrast ratio) and amber border/icon styling to resolve low-contrast yellow illegibility.
...
- **Category & Sort Toolbar Relocated Under Metrics Card (`[TASK-CONTROLS-UNDER-METRICS]`):** Moves the Category and Sort filter controls out of the top header and places them in a dedicated card directly beneath the "TASK PROGRESS & COMPLETION METRICS" card. Ensures an uncrowded header containing only the title, subtitle description, and desktop Add Task button, establishing an intuitive top-to-bottom hierarchy from high-level progress to granular board filtering.
- **Dynamic Assignee Filter with Unassigned Support (`[TASK-ASSIGNEE-FILTER]`):** Houses an interactive Assignee `<select>` dropdown (`ALL ASSIGNEES`, `UNASSIGNED`, and each distinct assigned collaborator) with live task counts. Operates concurrently with Category filtering across columns, mobile tabs, and progress completion metrics. Adding a new task while an assignee filter is active automatically pre-populates the assigned person in the Add Task modal.
- **Desktop Column Bottom Quick-Add Buttons (`[TASK-COLUMN-BOTTOM-ADD]`):** Pinned buttons (`+ ADD TO DO TASK`, `+ ADD IN PROGRESS TASK`, `+ ADD DONE TASK`) under each column list on desktop viewports. Automatically pre-populates the stage for the clicked column in the Add Task modal, providing quick task entry when scrolled to the bottom of long columns.
...
- **Dedicated Google Sheet Tab & Real-Time Sync (`[PHOTO-GUESTBOOK-SHEET-TAB]`):** Redesigned guest photo upload comment and note logging so submissions are saved directly to a dedicated `GUESTBOOK` tab in the couple's Google Sheet (`Entry ID`, `Date & Time`, `Guest Name`, `Message / Wishes`, `Photo Count`, `Photo Links`, `Drive Folder`). Resolves the issue where guest counts disappeared after refreshing the web app by designating Google Sheets as the single source of truth across serverless environments. Supports automatic tab auto-provisioning with formatted headers if the tab does not exist, bidirectional sync in `GET /api/sync` and `POST /api/sync`, direct upload writing in `POST /api/upload/[token]`, and immediate optimistic rendering with real-time sheet update triggers in `PhotoShotListManager.tsx`.
- **Permanent Offline Refresh Token Guarantee & Proactive Auto-Refresh (`[PHOTO-GUESTBOOK-OFFLINE-AUTH]`):** Guarantees persistent Google Drive connectivity for anonymous QR-code photo uploads beyond the standard 1-hour OAuth access token TTL. Enforces `prompt=consent` across Google OAuth entry points (`/api/auth/google`, `handleReauth`, `handleDirectGoogleAuth`) ensuring Google always returns a long-lived `refresh_token`. In `getGoogleAuthAsync`, detects expired or expiring access tokens (< 5 min remaining) and proactively calls `oauth2Client.getAccessToken()` to refresh credentials from Google's token endpoint, asynchronously writing the refreshed token and expiration timestamp back to Cloud Firestore and local storage. Integrates `sheet2suite-7036e` project ID detection in `getCloudFirestore()` to ensure serverless App Hosting instances access Firestore correctly without ephemeral storage fallback, and implements 401 retry resilience in `/api/upload/[token]`.
- **Long-Lived Production Sessions & Silent Refresh Resilience (`[AUTH-LONG-LIVED-SESSIONS]`):**
  - **90-Day `s2s_refresh_token` HTTP-Only Cookie**: Persists Google OAuth `refresh_token` in an HTTP-only, secure, `SameSite: 'lax'` cookie across `/api/auth/google/callback`, `POST /api/auth/google`, and `/api/auth/register-token`. Ensures that stateless Cloud Run containers in Firebase App Hosting can seamlessly refresh expired 1-hour access tokens without relying exclusively on ephemeral local disk or database round-trips.
  - **Dedicated Silent Refresh Route (`POST /api/auth/refresh`)**: Silently exchanges refresh tokens for fresh Google access tokens, updates session cookies, writes updated tokens to Cloud Firestore, and returns fresh tokens to callers.
  - **Automatic 401 Retry Resilience in `/api/sync` and `/api/auth/session`**: Catches 401 Unauthorized or expired credentials, automatically triggers `oauth2Client.getAccessToken()`, updates credentials, retries the Google Sheets operation once, and returns `freshAccessToken` in response payloads while updating the `s2s_access_token` cookie.
  - **Client-Side Silent Interceptor & Tab Visibility Heartbeat (`sessionCheck.ts`, `vow/page.tsx`)**: In `verifyActiveSession()`, `fetchWeddingData()`, and `pushToDatabase()`, catches 401 responses and triggers silent refresh via `/api/auth/refresh` before ever surfacing the "Session Expired" modal to the user. Synchronizes refreshed access tokens directly into `localStorage('s2v_google_token')` and React state. Proactively triggers silent token refresh on browser tab visibility regain and on a 25-minute interval timer. `handleReauth()` checks silent refresh first, resolving session dropouts in under 200ms without opening popup dialogs.
- **Guestbook Multi-Card Grid, Spreadsheet Table View & Serial Date Normalization (`[PHOTO-GUESTBOOK-UI-GRID-TABLE]`):**
  - **Serial Date & Timestamp Engine**: Normalizes Google Sheets / Excel unformatted serial timestamps (e.g. `46283.05763888889` $\rightarrow$ `Sep 18, 2026, 1:23 AM`) via `Math.round((serial - 25569) * 86400 * 1000)` in both `guestbookMapper.fromRow` (`mapper.ts`) and `formatUploadTime` (`PhotoShotListManager.tsx`), eliminating raw float date representations across device sync operations.
  - **Desktop Multi-Column Cards Grid**: Converted the guestbook cards feed from a single stretched column into a responsive CSS grid (`repeat(auto-fill, minmax(360px, 1fr))`), delivering a balanced 2-3 column gallery on desktop viewports.
  - **Cards vs Table / List View Toggle**: Added an integrated view mode toggle (`guestViewMode: 'card' | 'table'`) featuring `LayoutGrid` and `List` icons in the Guestbook toolbar adjacent to "With Written Notes Only", providing instant 1-click switching between visual cards and an organized tabular ledger.
